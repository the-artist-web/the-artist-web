...
const Page = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          const res = await fetch("/the-artist-web/docs/Sliderbar-ts.json");
          const data = await res.json();
          setData(data);
          setLoading(false);
        } catch (error) {
          console.error("Error Fetch Data In Type Script", error);
          setLoading(false);
        };
      };
    ...
    useEffect(() => {
      document.title = "type script | the artist web";
    ...
    return (
      <>
        <Navbar />
        <Aside />
        <SlideDocs file="Sliderbar-ts.json" />
        <Searchbar file="Sliderbar-ts.json" />
  
        <main className="main-docs">
          <article>
            <div className="docs-body">
              <div className="container">
                <p className="text-capitalize text-main label-small">docs / type script</p>
  
                <h1 className="text-capitalize mb-3 headline-small">type script Tutorial</h1>
  
                <p className="mb-4 label-small">TypeScript is an open source programming language that builds on JavaScript and adds additional features, most notably static typing. Developed by Microsoft, TypeScript provides a more powerful and maintainable development environment, making it popular for large and complex projects.</p>
